...
public class LiveStreamController {
    ...
    @PostMapping(value="/stop-stream", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public ResponseEntity<Object>stopStream(@RequestBody Map<String, String> json) throws Exception {
        String courseName = json.get("courseName");
        Courses course = coursesRepository.findByName(courseName);
        course.setIs_live(0);
        course.setAvailable_for_groups(null);
        coursesRepository.save(course);
        String status = "success";
        try {
            return new ResponseEntity<Object>(status, HttpStatus.OK);
